            ...
            <div class="header">
                ...
                >
                    ...
                    <el-button type="default" size="mini" @click="downloadFile"
                        >下载</el-button
                    >
                    <el-button type="default" size="mini" @click="shareFile"
                        >分享</el-button
                    >
                    <el-divider direction="vertical"></el-divider>
    ...
    methods: {
                        ...
                        this.startConnection()
                            .then(() => {
                                ...
                                this.editor.txt.html(
                                    StringUtil.strToUtf16(this.getDocData())
                                );
                                document
                                    .querySelector(".w-e-text-container")
                                    .setAttribute("style", "z-index: 500;");
                                document
                                    .querySelector(".w-e-toolbar")
                                    .setAttribute(
                                        "style",
                ...
                diff.forEach((op) => {
                    if (op.r !== undefined) {
                        ...
                        for (let i = 0; i < insertKey.length; i++) {
                            ...
                            if (index < oldContainer.children.length) {
                                oldContainer.insertBefore(
                                    tmpNode.children.item(0),
                                    oldContainer.children.item(index)
                                );
                            } else {
                        ...
                        for (let i = 0; i < deleteKey.length; i++) {
                            ...
                            if (StringUtil.isEmpty(text)) {
                                throw Error(
                                    "InsertLine 时发生异常，hashMap没有对应的字符串"
                                );
                            }
                        ...
                        for (let i = 0; i < oldKey.length; i++) {
                            ...
                            oldContainer.children.item(
                                index
                            ).outerHTML = map.map.get(newKey[i]);
                            index++;
                        }
                    }
                });
            ...
            if (diff.length !== 0) {
                if (diff.length <= 3) {
                    ...
                    if (compressOps !== null) {
                        ...
                        console.log(
                            `不压缩，直接提交：${JSON.stringify(diff)}`
                        );
                    }
                } else {
                    console.log(
                        `复杂操作，非更新式提交：：${JSON.stringify(diff)}`
                    );
                }
                ...
                DocumentsApi.writeFile(this.file.hash)
                console.log("变更已提交");
            } else {
                console.log("无变化");
            }
        ...
        fillSeq() {
            ...
            errorIndexList.forEach((i) => {
                ...
                element.children
                    .item(i)
                    .setAttribute("index", `${this.user.id}-${nextIndex++}`);
            });
            ...
            this.syncListener = setInterval(() => {
                ...
                if (docData !== containerData) {
                    ...
                    if (this.forceSyncCount < forceSyncLimit) {
                        console.warn(
                            `数据不同步，count:${this.forceSyncCount}`
                        );
                        this.forceSyncCount++;
                    } else {
                        console.error(
                            "数据不同步尝试次数达到上限，冻结编辑器，等待重新同步数据"
                        );
                        // this.syncData();
                        clearInterval(this.syncListener);
        ...
        insertLine(index, text) {
            this.doc.submitOp([
                "data",
                index,
                { i: { text: StringUtil.utf16ToStr(text) } },
            ]);
        },
        deleteLine(index) {
        ...
        updateLine(index, oldText, newText) {
            let diff = Ot.diff(
                StringUtil.utf16ToStr(oldText),
                StringUtil.utf16ToStr(newText)
            );
