        ...
        <div class="content">
            <div class="main" id="main">
                <div
                    ...
                    :style="{
                        flexDirection:
                            item.user.uid === user.id ? 'row-reverse' : 'row',
                    }"
                    :key="index"
                >
                    <el-avatar
                        :src="memberList[item.user.uid].avatar"
                    ></el-avatar>
                    <!-- <div
                        style="
                            display: flex;
                        ...
                        "
                    > -->
                        <!-- <span>{{ memberList[item.user.uid].username }}</span> -->
                        <p>{{ item.text }}</p>
                    <!-- </div> -->
                </div>
            </div>
            ...
            >
                <textarea
                    ...
                    v-model="text"
                ></textarea>
                <el-button size="mini" style="flex-grow: 1" @click="send"
                    >发送</el-button
                >
            </div>
        </div>
...
export default {
    ...
    methods: {
        updateMemberInfo(id) {
            return AuthApi.queryUserById(id);
        },
        drag(oDrag, handle) {
            let disX = 0;
            let disY = 0;
        ...
        receive(data) {
            this.updateMemberInfo(data.uid).then((r) => {
                let res = r.data;
                console.log(res.data);
                if (res.status === 200) {
                    this.memberList[data.uid] = res.data;
                    this.messages.push({
                        user: {
                            uid: data.uid,
                        },
        ...
        send() {
            if (this.text !== "") {
                this.updateMemberInfo(this.user.id).then((r) => {
                    let res = r.data;
                    console.log(res.data);
                    if (res.status === 200) {
                        this.memberList[this.user.id] = res.data;
                        this.messages.push({
                            user: {
                                uid: this.user.id,
                            },
...
#chatbox {
    ...
    border-radius: 3px;
    background: #ffffff;
    box-shadow: 5px 5px 13px #737373, -5px -5px 13px #ffffff;
    z-index: 1600;
}
