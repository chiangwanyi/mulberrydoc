...
var Ot = /** @class */ (function () {
     ...
     * @param a 当前字符串 after
     */
    Ot.prototype.diff = function (b, a) {
        var delta = new quill_delta_1.default().insert(b).diff(new quill_delta_1.default().insert(a));
        var ops = [];
        var index = 0;
        delta.ops.forEach(function (el) {
     ...
     * @param ops 操作
     */
    Ot.prototype.compress = function (ops) {
        var compressOps = [];
        // 只有操作数 >= 2 && <= 3 的操作可以压缩
     ...
     */
    Ot.prototype.stringsMap = function (bs, as) {
        // 字符串 -> 字符串映射
     ...
     * @param ops 操作（简写）
     */
    Ot.prototype.apply = function (str, ops) {
     ...
     * @param ops 操作（简写）
     */
    Ot.prototype.invert = function (ops) {
